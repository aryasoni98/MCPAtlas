...
use mcp_atlas_data::embeddings::{
    EmbeddingProvider, OllamaEmbeddingProvider, OpenAICompatibleEmbeddingProvider,
};
use mcp_atlas_data::models::Project;
use mcp_atlas_data::pipeline::{self, PipelineConfig};
use mcp_atlas_data::storage::{GraphBackend, VectorBackend};
use mcp_atlas_graph::engine::ProjectGraph;
...
async fn init_state(
    ...
    let cache_dir = config
        ...
        .unwrap_or_else(|| dirs_cache_dir().join("mcp-atlas"));
    let max_age = std::time::Duration::from_secs(config.max_cache_age);

    // Try loading from cache first
    let projects = if let Some(cached) = mcp_atlas_data::cache::load(&cache_dir, max_age) {
        cached
    } else {
        ...
        let projects = if config.skip_github {
            let (_landscape, projects) = match &pipeline_config.landscape_file {
                Some(path) => mcp_atlas_data::landscape::load_landscape_from_file(path)?,
                None => mcp_atlas_data::landscape::load_landscape().await?,
            };
            projects
        } else {
        ...
        if let Err(e) = mcp_atlas_data::cache::save(&cache_dir, &projects) {
    ...
    let graph: std::sync::Arc<dyn GraphBackend> = match config.graph_backend {
        ...
        GraphBackendKind::Surreal => {
            ...
            {
                let edges = mcp_atlas_graph::engine::compute_edges(&projects);
                std::sync::Arc::new(
                    mcp_atlas_graph_surrealdb::SurrealGraphBackend::new(&edges)
                        .await
    ...
    let (provider, backend) = match (
        ...
        (Some(api_base), Some(qdrant_url)) => {
            ...
            let backend: Option<Arc<dyn VectorBackend>> = {
                let b = mcp_atlas_vector::QdrantVectorBackend::new(qdrant_url.as_str(), None, dims)
                    .await
...
pub fn build_router(state: Arc<AppState>) -> axum::Router {
    ...
    async fn handle_metrics(State(state): State<SharedState>) -> Response {
        ...
        let graph_stats = state.graph.stats().await.unwrap_or_else(|e| {
            tracing::warn!("graph stats failed: {e}");
            mcp_atlas_data::storage::GraphStats {
                total_nodes: 0,
                total_edges: 0,
        ...
        let body = format!(
            "# HELP mcp_atlas_projects_total Total number of CNCF projects loaded.\n\
             # TYPE mcp_atlas_projects_total gauge\n\
             mcp_atlas_projects_total {}\n\
             # HELP mcp_atlas_search_docs_total Total documents in the search index.\n\
             # TYPE mcp_atlas_search_docs_total gauge\n\
             mcp_atlas_search_docs_total {}\n\
             # HELP mcp_atlas_requests_total Total JSON-RPC requests processed.\n\
             # TYPE mcp_atlas_requests_total counter\n\
             mcp_atlas_requests_total {}\n\
             # HELP mcp_atlas_active_sessions Number of active Streamable HTTP sessions.\n\
             # TYPE mcp_atlas_active_sessions gauge\n\
             mcp_atlas_active_sessions {}\n\
             # HELP mcp_atlas_graph_nodes Total nodes in the knowledge graph.\n\
             # TYPE mcp_atlas_graph_nodes gauge\n\
             mcp_atlas_graph_nodes {}\n\
             # HELP mcp_atlas_graph_edges Total edges in the knowledge graph.\n\
             # TYPE mcp_atlas_graph_edges gauge\n\
             mcp_atlas_graph_edges {}\n\
             # HELP mcp_atlas_uptime_seconds Server uptime in seconds.\n\
             # TYPE mcp_atlas_uptime_seconds gauge\n\
             mcp_atlas_uptime_seconds {}\n",
            state.projects.len(),
            state.search_index.doc_count(),
            requests,
            sessions,
