...
pub fn handle_suggest_stack(state: &Arc<AppState>, args: &Value) -> Result<Value> {
    ...
    for subcat in &matched_subcats {
        ...
        candidates.sort_by(|a, b| {
            let maturity_ord = |p: &mcp_atlas_data::models::Project| match &p.maturity {
                Some(mcp_atlas_data::models::Maturity::Graduated) => 0,
                Some(mcp_atlas_data::models::Maturity::Incubating) => 1,
                Some(mcp_atlas_data::models::Maturity::Sandbox) => 2,
                _ => 3,
            };
...
pub fn handle_analyze_trends(state: &Arc<AppState>, args: &Value) -> Result<Value> {
    ...
    let graduated = projects_in_cat
        ...
        .filter(|p| {
            matches!(
                p.maturity,
                Some(mcp_atlas_data::models::Maturity::Graduated)
            )
        })
        .count();
    let incubating = projects_in_cat
        ...
        .filter(|p| {
            matches!(
                p.maturity,
                Some(mcp_atlas_data::models::Maturity::Incubating)
            )
        })
        .count();
    let sandbox = projects_in_cat
        .iter()
        .filter(|p| matches!(p.maturity, Some(mcp_atlas_data::models::Maturity::Sandbox)))
        .count();
...
pub async fn handle_get_migration_path(state: &Arc<AppState>, args: &Value) -> Result<Value> {
    ...
    lines.push(format!(
        "| Category | {} > {} | {} > {} |",
        from_p.category, from_p.subcategory, to_p.category, to_p.subcategory
    ));
    lines.push(format!(
        "| Maturity | {} | {} |",
        from_p
